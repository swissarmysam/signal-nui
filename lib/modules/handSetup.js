...
async function handleGesture(hand) {
  ...
  const gestures = {
    ...
    viewMode:
      ...
      !pinkyExtended,
    // close modal
    close: thumbExtended && pinkyExtended && !indexExtended && !middleExtended && !ringExtended,
  };
  ...
  if (gestureState === 1) {
    ...
    video.classList.remove('is-zero-zero');

    // create hacky timer for countdown
    timerEl.classList.remove('is-zero-zero');
    ...
    setTimeout(function() {
      // console.log('reset');
      video.classList.add('is-zero-zero');
