...
const isDigitExtended = (tip, base) => base > tip;
...
async function handleGesture(hand) {
  // console.log(hand);
  // get the current page for handling appropriate gestures
  ...
  const thumbBaseY = hand[0].landmarks[3][1];
  // index finger
  ...
  const thumbExtended = isDigitExtended(thumbTipY, thumbBaseY);
  ...
  // object of all motion gestures - each key is effectively true
  const gestures = {
    // wave
    notifyApp:
    ...
    // thumb down - not working properly - refresh only with voice as it detects false positive
    // refresh:
    /*  !thumbExtended &&
      !indexExtended &&
      middleExtended &&
    ...
    // !Failed to execute 'requestFullscreen' on 'Element': API can only be initiated by a user gesture. - can't do full view mode - would need to mimic behaviour with css class width and height
    /* viewMode:
      indexMiddleDiff > 30 &&
      indexExtended &&
      middleExtended &&
      !ringExtended &&
      !pinkyExtended, */
    // close modal - clench fist
    close: !pinkyExtended && !indexExtended && !middleExtended && !ringExtended,
  };
    ...
    if (currentPage === 'home') {
      if (gestures.navLeft) {
        ...
        if (activeSlide.querySelector('a:nth-child(2)')) {
          ...
          triggerEvent('click', '.swiper-slide-active', 'a', 'select');
        }
      }
    }
    ...
    if (currentPage === 'content') {
      ...
      if (!document.querySelector('.modal').classList.contains('is-active')) {
        if (gestures.select) {
          ...
          // await wait(1250);
          triggerEvent('click', '.back', 'a', 'back');
        }
      } else if (
        // modal (video) is open
